library(car) #Anova()
library(MASS) #model comparison
library(psych) #scatterplot of fm
library(ggplot2) #plotting
library(ggpubr) #stat compare means
...
rm(fm.model, fm.model2, occ.dfa, occ.dfa2, predicted, predicted2, test, test2, train, train2, train.idx, train.idx2)




######################## PLOTS: ANOVA
# Reusable function for boxplot with individual points and stats
# stat_compare_means adds pairwise significance annotations
...
#### FORAMEN MAGNUM
# Note: using combined data to include Horticultural group
...
#### need original foramen measures
occ.orig.fm = read.csv("revised-preparation-original-noIndex.csv") |> select(Subsistence,Sex, ForamenMagnumS,ForamenMagnumT)

occ.mined.fm = read.csv("data-occ-mined.csv") |>  mutate(Subsistence = case_when(
    Group == "Arikara"  ~ "Horticultural",
    Group == "Study"  ~ "Agriculture"))|> select(Subsistence,Sex,  ForamenMagnumS,ForamenMagnumT)

#merge
occ.fm = rbind(occ.mined.fm, occ.orig.fm)

#factors
occ.fm$SexN = as.numeric(occ.fm$Sex)
occ.fm$SubsistenceN = as.numeric(occ.fm$Subsistence)
